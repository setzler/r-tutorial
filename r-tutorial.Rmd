...
# Introduction to 4 Workhorse Objects 
...
# Data Analysis, Visualization, and Import/Export
...
2. Computing aggregate statistic;
...
The tidyverse coding style, which is derived from Google's R style guide, is available here: <http://style.tidyverse.org/>. However, it does not cover issues of scoping or when to check for erros, which are key.
...
Rewrite the codes you wrote for Quiz #1 and #2 to satisfy all of the style and structure rules. Save these as separate files so you can compare before/after. [Hint: the `style_active_file()` function from the *styler* package can do a lot of the work.]
...
# Developing a Package (IN PROGRESS)
